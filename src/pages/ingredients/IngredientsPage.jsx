...
export default function IngredientsPage() {
  ...
  function openCreate() {
    setEditing(null)
    reset({ name: '', quantity: '', unit: '', minStock: '' })
    setError('')
    setModalOpen(true)
  }
  ...
  async function onSubmit(data) {
    try {
      setError('')
      if (editing) {
        await ingredientService.update(editing.id, data)
      } else {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-semibold text-zinc-800">Insumos</h1>
          <p className="text-sm text-zinc-500 mt-0.5">Gerencie o estoque de ingredientes</p>
        </div>
              ...
              {ingredients.map((ing, i) => (
                <tr key={ing.id}
                  className={`border-b border-zinc-50 hover:bg-zinc-50 transition-colors
                    ${i === ingredients.length - 1 ? 'border-0' : ''}`}>
                  <td className="px-4 py-3 font-medium text-zinc-700">{ing.name}</td>
                  <td className="px-4 py-3 text-zinc-600">
                    <span className={ing.quantity <= ing.minStock
        ...
        <Modal title={editing ? 'Editar insumo' : 'Novo insumo'} onClose={closeModal}>
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Input label="Quantidade" type="number" step="0.001"
                {...register('quantity')} error={errors.quantity?.message} />
              <Input label="Unidade" placeholder="kg, litro..."
                {...register('unit')} error={errors.unit?.message} />
            </div>
            <Input label="Estoque mínimo" type="number" step="0.001"
              {...register('minStock')} error={errors.minStock?.message} />
