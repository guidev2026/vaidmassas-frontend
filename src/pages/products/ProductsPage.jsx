...
export default function ProductsPage() {
  ...
  useEffect(() => {
    Promise.all([categoryService.getAll(), ingredientService.getAll()])
      .then(([cats, ings]) => { setCategories(cats); setIngredients(ings) })
  }, [])

  useEffect(() => { load() }, [filterCat])

  async function load() {
    try { setLoading(true); setProducts(await productService.getAll(filterCat || null)) }
    finally { setLoading(false) }
  }

  function openCreate() {
    setEditing(null)
    reset({ name: '', price: '', categoryId: '', ingredients: [{ ingredientId: '', quantity: '', unit: '' }] })
  ...
  function openEdit(p) {
    setEditing(p)
    reset({
      name: p.name, price: p.price, categoryId: p.category.id,
      ingredients: p.ingredients.map(i => ({ ingredientId: i.ingredientId, quantity: i.quantity, unit: i.unit })),
    })
    setError(''); setModalOpen(true)
  }
  ...
  async function onSubmit(data) {
    try {
      setError('')
      editing ? await productService.update(editing.id, data)
  ...
  async function handleDelete(id) {
    ...
    await productService.remove(id); await load()
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h1 className="text-lg md:text-xl font-semibold text-zinc-800">Produtos</h1>
          <p className="text-xs md:text-sm text-zinc-500 mt-0.5">Cardápio com ficha técnica</p>
        </div>
      ...
      <div className="mb-4">
        <select value={filterCat} onChange={e => setFilterCat(e.target.value)}
          className="w-full md:w-auto border border-zinc-300 rounded-lg px-3 py-2 text-sm text-zinc-700 outline-none focus:ring-2 focus:ring-orange-400">
        ...
        <>
          <div className="hidden md:block bg-white rounded-xl border border-zinc-200 overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {products.length === 0 && (
                  <tr><td colSpan={5} className="text-center py-8 text-zinc-400">Nenhum produto cadastrado</td></tr>
                )}
                {products.map((p, i) => (
                  <tr key={p.id} className={`border-b border-zinc-50 hover:bg-zinc-50 ${i === products.length - 1 ? 'border-0' : ''}`}>
                    <td className="px-4 py-3 font-medium text-zinc-700">{p.name}</td>
                    <td className="px-4 py-3">
                      <span className="bg-orange-100 text-orange-700 text-xs px-2 py-0.5 rounded-full">{p.category.name}</span>
                    </td>
                    <td className="px-4 py-3 text-zinc-700">R$ {Number(p.price).toFixed(2)}</td>
                    <td className="px-4 py-3 text-zinc-500 text-xs">{p.ingredients.map(i => i.ingredientName).join(', ')}</td>
            ...
            {products.map(p => (
              <div key={p.id} className="bg-white rounded-xl border border-zinc-200 p-4">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <span className="font-semibold text-zinc-700">R$ {Number(p.price).toFixed(2)}</span>
                </div>
                <span className="bg-orange-100 text-orange-700 text-xs px-2 py-0.5 rounded-full">{p.category.name}</span>
                <div className="text-xs text-zinc-400 mt-2 mb-3">{p.ingredients.map(i => i.ingredientName).join(', ')}</div>
                  ...
                  <div key={field.id} className="flex gap-2 items-start">
                    <div className="flex-1 min-w-0">
                      <select {...register(`ingredients.${index}.ingredientId`)}
                        className="w-full border border-zinc-300 rounded-lg px-2 py-2 text-sm outline-none focus:ring-2 focus:ring-orange-400">
                    ...
                    {fields.length > 1 && (
                      <button type="button" onClick={() => remove(index)}
                        className="text-red-400 hover:text-red-600 text-xl leading-none mt-1.5 flex-shrink-0">&times;</button>
                    )}
                  </div>
                ))}
              </div>
